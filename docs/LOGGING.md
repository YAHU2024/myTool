...
1. 打开 QuickTranslate 设置。
2. 展开默认折叠的“开发者选项”。
3. 点击“查看本地日志”。

日志查看器采用单窗口模式。窗口已经打开时再次点击“查看本地日志”，应用会激活现有窗口，不会创建第二个实例。关闭日志查看器不会停止后台日志写入。普通用户报告问题请使用托盘“反馈问题”或设置“帮助与反馈”；日志不会自动上传。
...
打开托盘菜单中的“设置”，展开“开发者选项”配置日志。展开该区域不会改变任何日志配置。
