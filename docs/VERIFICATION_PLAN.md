...
> 自动化基础：单元测试 219/219 通过（含 12 项 AuthenticodeVerifier 新测试）
...
| 场景 | 操作 | 预期行为 |
|:-----|:-----|:---------|
| **正常签名安装包** | 签名文件 + 正确发布者 | 弹出下载进度 → 签名验证通过 → 提示 UAC → 安装 |
| **无签名安装包** | 未签名文件 | 弹出下载进度 → 签名验证失败 → 显示错误 → 安装被阻止 |
| **错误发布者签名** | 用不同证书签名 | 弹出下载进度 → PublisherMismatch → 安装被阻止 |
| **SHA256 不匹配** | 下载后文件损坏 | 弹出下载进度 → "校验失败" → 安装被阻止 |
