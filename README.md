...
如果模型在任一模式的流式响应中明确返回 reasoning 字段，悬浮窗会在对应回答下显示 ChatGPT 风格的“思考”区域：生成期间展开，完成后收起，可用上下箭头查看或隐藏。思考最多保留 8,000 个 Unicode 标量，不提供复制，仅存在于当前会话，不会进入最终答案、历史、缓存或日志；模型不支持时该区域不会出现。
